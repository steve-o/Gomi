...
static const char* kSnmpApplicationName = "Gomi";

LONG volatile gomi::snmp_agent_t::ref_count_ = 0;

class gomi::snmp::event_pump_t
{
public:
...
gomi::snmp_agent_t::snmp_agent_t (gomi::gomi_t& gomi) :
	gomi_ (gomi)
...
gomi::snmp_agent_t::~snmp_agent_t (void)
{
	clear();
}

bool
gomi::snmp_agent_t::run (void)
{
/* Instance already running. */
	if (InterlockedExchangeAdd (&ref_count_, 1L) > 0)
		return true;

	if (gomi_.config_.is_agentx_subagent)
	{
		LOG(INFO) << "Configuring as SNMP AgentX sub-agent.";
		if (!gomi_.config_.agentx_socket.empty())
		{
			LOG(INFO) << "Using AgentX socket " << gomi_.config_.agentx_socket << '.';
			::netsnmp_ds_set_string (NETSNMP_DS_APPLICATION_ID,
						 NETSNMP_DS_AGENT_X_SOCKET,
						 gomi_.config_.agentx_socket.c_str());
		}
	...
	if (!gomi_.config_.snmp_filelog.empty()) {
		LOG(INFO) << "Setting Net-SNMP filelog to \"" << gomi_.config_.snmp_filelog << "\"";
		::snmp_enable_filelog (gomi_.config_.snmp_filelog.c_str(), 0);
	}
